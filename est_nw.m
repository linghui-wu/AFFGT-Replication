...
[w_j] = deal(x_squared(1));
[M_u_i, M_u_j, M_d_i, M_d_j] = deal(x_squared(2), x_squared(3), ...
                                x_squared(4), x_squared(5));
[T_i, T_j] = deal(x_squared(6), x_squared(7));
...
mc_u_j = alpha_u_bar / A_u_j * x(1) ^ 2 ^ alpha_u; % * (P_u_j) ^ 0
...
P_u_ji = x(3) ^ 2 ^ (1 / (1 - theta)) * (1 + t_u_ji) * p_u_ji;
P_u_ij = x(2) ^ 2 ^ (1 / (1 - theta)) * (1 + t_u_ij) * p_u_ij;
P_u_ii = x(2) ^ 2 ^ (1 / (1 - theta)) * (1 + t_u_ii) * p_u_ii;
P_u_jj = x(3) ^ 2 ^ (1 / (1 - theta)) * (1 + t_u_jj) * p_u_jj;
...
mc_d_j = alpha_d_bar / A_d_j * x(1) ^ 2 ^ alpha_d * P_u_j ^ (1 - alpha_d);
...
P_d_ji = x(5) ^ 2 ^ (1 / (1 - sigma)) * (1 + t_d_ji) * p_d_ji;
P_d_ij = x(4) ^ 2 ^ (1 / (1 - sigma)) * (1 + t_d_ij) * p_d_ij;
P_d_ii = x(4) ^ 2 ^ (1 / (1 - sigma)) * (1 + t_d_ii) * p_d_ii;
...
l_d_j = alpha_d * mc_d_j * (f_d + y_d_j) / x(1) ^ 2;
...
c_jj = (x(1) ^ 2 * L_j + x(7) ^ 2) * ((1 + t_d_jj) * p_d_jj) ^ (-sigma) / P_d_j ^ (1 - sigma);
c_ij = (x(1) ^ 2 * L_j + x(7) ^ 2) * ((1 + t_d_ij) * p_d_ij) ^ (-sigma) / P_d_j ^ (1 - sigma);
c_ii = (w_i * L_i + x(6) ^ 2) * ((1 + t_d_ii) * p_d_ii) ^ (-sigma) / P_d_i ^ (1 - sigma);
% Labor market clearing
LMC_i = L_i - x(4) ^ 2 * l_d_i - x(2) ^ 2 * l_u_i;
LMC_j = L_j - x(5) ^ 2 * l_d_j - x(3) ^ 2 * l_u_j;
% Goods market clearinge
GMC_d_i = y_d_i - c_ii - tau_d * c_ij;
GMC_d_j = y_d_j - c_jj - tau_d * c_ji;
GMC_u_i = y_u_i - x(4) ^ 2 * x_ii - x(5) ^ 2 * tau_u * x_ij;
GMC_u_j = y_u_j - x(5) ^ 2 * x_jj - x(4) ^ 2 * tau_u * x_ji;
